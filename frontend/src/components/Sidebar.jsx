...
function Sidebar({
  ...
  const inner = (
    <div className="flex flex-col h-full">
      <div
        className={`px-3 h-[52px] border-b border-black/8 shrink-0 flex 
        items-center ${sidebarOpen ? "justify-between" : "justify-center"}`}
      >
        {sidebarOpen && (
          <div className="flex items-center gap-2.5">
            <div
              className="w-7 h-7 rounded-lg bg-[#000000] flex 
                    items-center justify-center shrink-0 shadow-[0_4px_14px_rgba
                    (0,0,0,0.25)]"
            >
            ...
            <motion.span
              ...
              className="font-extrabold text-sm tracking-tighter
                        [#0A0A0A] whitespace-nowrap"
            >
              FresherAI
            </motion.span>
          </div>
      ...
      <div className="px-2.5 pt-3 pb-1.5 shrink-0">
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className={`w-full flex items-center gap-2 bg-[#000000]
        text-white font-semibold rounded-lg py-2 transition-all
        shadow-[0_4px_14px_rgba(0,0,0,0.25)] cursor-pointer hover:bg-[#1a1a1a] ${
          ...
          <AnimatePresence>
            {sidebarOpen && (
              <motion.span
                ...
                className="text-xs whitespace-now"
              >
                Create Interview
              </motion.span>
            )}
      ...
      <nav className="flex flex-col gap-0.5 px-2.5 flex-1">
        {NAV_ITEMS.map((nav, i) => (
          <motion.button
            ...
            transition-all text-xs font-medium text-black/45 hover:text-[#0A0A0A] hover:bg-black/5 cursor-pointer ${
              sidebarOpen ? "px-2.5 " : "justify-center px-0"
            }`}
          >
      ...
      <div className="border-t border-black/8 p-2.5 shrink-0">
        <AnimatePresence>
          {sidebarOpen && (
            <motion.div
              ...
              className="group flex cursor-pointer items-center justify-between gap-2.5
                rounded-lg border border-white/10 bg-[#000000]/90 backdrop-blur-2xl px-2.5 py-1.5
                mb-2.5 transition-all hover:border-white/20 shadow-[0_8px_24px_rgba(0,0,0,0.2)]"
            >
        ...
        >
          <div
            className="w-7 h-7 rounded-full bg-[#000000] flex items-center justify-center
    shrink-0"
          >
