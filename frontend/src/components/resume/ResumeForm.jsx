  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        ...
        value={value}
        className="bg-white border-2 border-black/25 text-[#0A0A0A] text-xs rounded-lg px-2.5 py-2 focus:border-none focus:outline-none focus:ring-2 focus:ring-black/10 transition-colors placeholder-black/30 shadow-[0_2px_8px_rgba(0,0,0,0.04)]"
      />
    </div>
  );
}
...
function EntryCard({ children, onRemove }) {
  return (
    <div className="relative overflow-hidden bg-[#F8F9FA] border-2 border-black/15 rounded-xl p-3 shadow-[0_2px_10px_rgba(0,0,0,0.05)]">
      <button
        className="absolute top-2.5 right-2.5 text-black/35 hover:text-red-500 transition-colors"
        onClick={onRemove}
      >
        <FiTrash2 size={13} />
      </button>
        ...
        {data.projects.map((pro, index) => (
          <EntryCard key={index} onRemove={() => removePro(index)}>
            ...
            <Input
              ...
              placeholder="github.com/rahul/interviewiq"
            />
            ...
            <TextArea
              ...
              placeholder="AI-powered interview preparation platform with mock interviews and resume analysis features."
            />
          </EntryCard>
        ))}
